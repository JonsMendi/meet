...
const getToken = async (code) => {
    ...
    const { access_token } = await fetch('https://r3t05ukc97.execute-api.eu-central-1.amazonaws.com/dev/api/token' + '/' + encodeCode)
    .then((res) => { res.json(); })
    .catch((error) =>  error);
...
export const getEvents = async () => {
    ...
    if (token) {
        removeQuery();
        const url = 'https://r3t05ukc97.execute-api.eu-central-1.amazonaws.com/dev/api/get-events' + '/' + token;
...
export const getAccessToken = async () => {
    ...
    if (!accessToken || tokenCheck.error) {
        ...
        if (!code) {
            const results = await axios.get('https://r3t05ukc97.execute-api.eu-central-1.amazonaws.com/dev/api/get-auth-url');
